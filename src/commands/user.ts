import { CommandInteraction, Client, ApplicationCommandType, ApplicationCommandOptionType, EmbedBuilder, inlineCode } from "discord.js";
...
export const user: Command = {
  ...
  run: async (interaction: CommandInteraction) => {
    const steamId = interaction.options.data.find(option => option.name === 'steamid')?.value as string
    const id = interaction.options.data.find(option => option.name === 'id')?.value as number

    if (!steamId && !id) {
      await interaction.reply({ content: 'You must provide either a Steam ID or a user ID.', ephemeral: true })
      return
    }

    try {
      const user = await getUser({ steamId, id });
      const allValidRecords = await getRecords({ UserSteamId: steamId, UserId: id, Limit: 0 });
      const allInvalidRecords = await getRecords({ UserSteamId: steamId, UserId: id, ValidOnly: false, Limit: 0 });
      const bestRecords = await getRecords({ UserSteamId: steamId, UserId: id, BestOnly: true, Limit: 0 });
      const worldRecords = await getRecords({ UserSteamId: steamId, UserId: id, WorldRecordOnly: true, Limit: 5 });

      const totalRuns = allValidRecords.totalAmount + allInvalidRecords.totalAmount;

      const worldRecordsList = worldRecords.records.map(record => {
        return `${record.level.name} by ${record.level.author}`
      }).join('\n')

      const worldRecordsTimeList = worldRecords.records.map(record => {
