...
const thundernerdKofiLink = hyperlink(
  'Ko-fi/thundernerd',
  'https://ko-fi.com/thundernerd'
)

const botGithubLink = hyperlink(
  'GitHub',
  'https://github.com/zeepkist/zeepkist-bot'
)

const zeepkistSteamLink = hyperlink(
  'Zeepkist',
...
const zeepkistSuperLeagueLink = hyperlink(
  'Super League results',
  'https://zeepki.st/super-league'
)

const modkistDownloadLink = hyperlink(
  'Modkist (Zeepkist Mod Manager)',
  'https://github.com/tnrd-org/ModkistRevamped/releases/latest/download/Modkist.-.Revamped.zip'
)

const gtrGithubLink = hyperlink('GitHub', 'https://github.com/tnrd-org')

const gtrApiLink = hyperlink(
  'GTR API',
  'https://graphql.zeepkist-gtr.com/graphiql'
)

const zworpshopApiLink = hyperlink(
  'Zworpshop API',
  'https://graphql.zworpshop.com/graphiql'
)

const zeepkistApiLink = hyperlink('zeepki.st API', 'https://zeepki.st/graphiql')

const steamApiLink = hyperlink(
  'Steam API',
...
embed.setDescription(
  `A Discord bot for ${zeepkistSteamLink} records (GTR PBs/WRs), ${zeepkistSuperLeagueLink} and playlist generation.\n\nAll data is provided by the ${gtrApiLink}, the ${zworpshopApiLink}, the ${zeepkistApiLink} and the ${steamApiLink}.\n\nThe bot is not affiliated with Steam and only uses the Steam API to get ${bold(
    'public'
  )} user information not provided by GTR. Data obtained from the Steam API is never stored.${spacer}`
)
