...
export const paginatedRecent = async (properties) => {
    ...
    const worldRecordOnly = messageData
        ? JSON.parse(messageData[0].query).worldRecordsOnly
        : query?.worldRecordsOnly ?? false;
    let { totalPages, currentPage } = extractPages(isUpdate ? interaction.message.embeds[0].footer?.text : undefined);
    ...
    log.info(interaction, `Obtained ${totalAmount} recent records. Showing page ${currentPage} of ${totalPages}`);
    ...
    const embed = new EmbedBuilder()
        ...
        .setTimestamp();
    const pagination = paginationButtons(interaction, 'recent', currentPage, totalPages);
    if (isUpdate) {
        log.info(interaction, `Updating message ${interaction.message.interaction?.id}`);
