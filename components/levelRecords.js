...
import { bestMedal, formatRelativeDate, formatResultTime, MEDAL, providedBy, userSimilarity } from '../utils/index.js';
import { listRecords } from './lists/listRecords.js';
export const levelRecords = async (interaction, level, page = 1, limit = 10) => {
    ...
    handleUserRecords: if (user && user.length > 0) {
        ...
        const personalBest = `${bestMedal(record)} ${inlineCode(formatResultTime(record.time))}\n${formatRelativeDate(record.dateCreated)} with ${userRecord.totalAmount} total runs`;
        embed.addFields({
    ...
    if (records.totalAmount > limit) {
        ...
        if ((!user || user.length === 0) &&
            userSimilarity(interaction.user.username, records.records.map(({ user }) => user.steamName)) > 3) {
            bestRecords += `\n\nLink your Steam ID with ${inlineCode('/verify')} to always see your personal best.`;
        }
    }
    embed.addFields({
        name: 'Best Times',
