...
export const levelRecords = async (interaction, level, page = 1, limit = 10) => {
    ...
    const medalTimes = [
        level.timeAuthor &&
            `<:zeepkist_author:1008786679173234688> ${inlineCode(formatResultTime(level.timeAuthor))}`,
        level.timeGold &&
            `<:zeepkist_gold:1008786743706783826> ${inlineCode(formatResultTime(level.timeGold))}`,
        level.timeSilver &&
            `<:zeepkist_silver:1008786769380130959> ${inlineCode(formatResultTime(level.timeSilver))}`,
        level.timeBronze &&
            `<:zeepkist_bronze:1008786713688166400> ${inlineCode(formatResultTime(level.timeBronze))}`
    ].join('\n');
