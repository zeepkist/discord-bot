...
export const userRankings = async (interaction, page = 1, limit = 10) => {
    ...
    console.log(`[ranking]: Obtained ${rankings.rankings.length} users. ${page}/${totalPages} pages.`);
    const rankingsList = rankings.rankings
        .map(ranking => {
        const rankingNumber = bold(`${ranking.position}.`);
        const rankingUser = hyperlink(ranking.user.steamName, `https://zeepkist.wopian.me/user/${ranking.user.steamId}`);
