...
import { TwitchStats, TwitchUserStats } from '../../models/twitch.js'
import { formatOrdinal } from '../../utils/format.js'

const twitchUserStats = (stats: TwitchUserStats) =>
  `${hyperlink(
    stats.userName,
    `https://twitch.tv/${stats.userName}`
  )} (${Math.floor(stats.value)})`

export const twitchStatsEmbed = (stats: TwitchStats) => {
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      },
      {
        ...
        value: `${stats.mostDailyViewers} (${formatOrdinal(
          stats.mostDailyViewersDay
        )})`,
        inline: true
      },
      {
